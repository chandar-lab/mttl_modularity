...
def my_train_model_acce(args, model, datamodule, do_test=False, checkpoint_path=None, resume_from_checkpoint=False):
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        # dataloader = datamodule.train_dataloader()
        # print('@@@@@@@@@@@@@@@@@@@@@@@ inside the epoch: @@@@@@@@@@@@@@@@ len(dataloader) is', len(dataloader))
        # total_steps = len(dataloader) // args.gradient_accumulation_steps
        
        # progress_bar = tqdm(range(total_steps), desc=f"Epoch {epoch+1}/{args.num_train_epochs}")
        step = 0
        with tqdm(total=steps_per_epoch, desc=f"Epoch {epoch+1}/{args.num_train_epochs}") as bar:
