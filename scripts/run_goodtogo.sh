...
function _resubmit_on_timeout() {
    ...
    if [ -f "$FINAL_CHECKPOINT" ]; then
        echo "Final checkpoint already exists at $FINAL_CHECKPOINT — skipping requeue."
        exit 0
    else
        echo "Last Checkpoint not found. Requeueing job..."
        trap "" SIGTERM
        scancel -s SIGTERM --full $SLURM_JOBID
        scontrol requeue ${SLURM_JOB_ID} 
    fi
...
if [ -d "${CHECKPOINT_PATH}checkpoint-2/" ]; then
...
TORCH_DISTRIBUTED_DEBUG=DETAIL accelerate launch main_pipline.py train_config.model=$MODEL \
